...
function dodajUcenika(
    ...
    $datum_rodjenja, $jmbg, $spol, $adresa, $mjesto,
    $skolska_godina, $razred, $udaljenost, $iznos,
    $banka, $ziro_racun, $ustanova, $status_uplate,
    $status, $napomena, $nosilac_racuna = null, $jmbg_roditelja = null
){
    $conn = dbConnect();
    ...
    $jmbg_roditelja = preg_replace('/\D+/', '', (string)$jmbg_roditelja);
    if ($jmbg_roditelja === '') { $jmbg_roditelja = null; }

    // 22 kolone / 22 placeholdera
    $sql = "INSERT INTO ucenici (
                ime_ucenika, prezime_ucenika, ime_oca, ime_majke, prezime_majke,
                jmbg, datum_rodjenja, spol, mjesto_stanovanja, adresa_stanovanja,
                udaljenost_do_skole, razred, banka, broj_racuna, iznos,
                status_uplate, ustanova_id, id_godine, status, napomena,
                nosilac_racuna, jmbg_roditelja
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

    $stmt = $conn->prepare($sql);
    // Tipovi: s=string, i=int, d=double
    $stmt->bind_param(
        "ssssssssssisssdiiiisss",
        $ime, $prezime, $ime_oca, $ime_majke, $prezime_majke,
        $jmbg, $datum_rodjenja, $spol, $mjesto, $adresa,
        $udaljenost, $razred, $banka, $ziro_racun, $iznos,
        $status_uplate, $ustanova, $skolska_godina, $status, $napomena,
        $nosilac_racuna, $jmbg_roditelja
    );
...
function izmjenaStatusaOdobrenja($id_ucenika, $status, $napomena, $nosilac_racuna = null, $jmbg_roditelja = null) {
    ...
    // Sanitizacija / validacija
    $nosilac_racuna = trim((string)$nosilac_racuna);
    if ($nosilac_racuna !== '' && mb_strlen($nosilac_racuna) > 80) {
        $nosilac_racuna = mb_substr($nosilac_racuna, 0, 80);
    } elseif ($nosilac_racuna === '') {
        $nosilac_racuna = null;
    }

    $jmbg_roditelja = preg_replace('/\D+/', '', (string)$jmbg_roditelja);
    if ($jmbg_roditelja !== '' && !preg_match('/^\d{13}$/', $jmbg_roditelja)) {
        $jmbg_roditelja = null;
    } elseif ($jmbg_roditelja === '') {
        $jmbg_roditelja = null;
    }

    $sql = "UPDATE ucenici
            SET status = ?, napomena = ?, nosilac_racuna = ?, jmbg_roditelja = ?
            WHERE id_ucenika = ?";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isssi", $status, $napomena, $nosilac_racuna, $jmbg_roditelja, $id_ucenika);
